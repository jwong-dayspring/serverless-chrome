...
function cleanPrintOptionValue (type, value) {
  console.log('cleanPrintOptionValue', type, value)
...
export default (async function printToPdfHandler (event) {
  const { body: { src, ...printParameters } } = event
  const printOptions = makePrintOptions(printParameters)
  let pdf

  console.log(event)
  console.log('making pdf for content', src)
  log('Processing PDFification for', printOptions)

  const startTime = Date.now()

  try {
    pdf = await printContentToPdf(src, printOptions)
  } catch (error) {
